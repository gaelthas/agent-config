...
### 2.1 团队冲突规避：配置标识（Namespace）

如果团队成员同时使用多套 Claude Code 配置（或担心与 Claude Code 自带配置混淆），建议为配置集定义唯一标识，并约定统一的调用标记。

本仓库的约定：

- 配置代号：`UCC`
- 调用标记：`@ucc`
- 推荐显式团队入口：`/ucc-team`、`/ucc-team-fast`、`/ucc-team-strict`
- 当请求包含 `@ucc` 或通过本仓库 `commands/` 触发斜杠命令时，输出末尾应出现：`配置标识：UCC`
...
| `typescript-reviewer.md` | TypeScript/Vue 前端审查 | 使用 TS/Vue 则保留 |
| `typescript-backend-reviewer.md` | TypeScript/Node 后端审查 | 使用 TS 后端则保留 |
| `typescript-fullstack-reviewer.md` | TypeScript 全栈审查（monorepo）| 前后端同仓则保留 |
| `refactor-cleaner.md` | 死代码清理 | 重构时保留 |
| `database-reviewer.md` | MySQL/SQLite 数据库审查 | 涉及数据库则保留 |
| `design-doc-writer.md` | 设计文档生成 | 需要设计文档输出则保留 |
| `delivery-doc-writer.md` | 交付文档生成 | 项目交付时保留 |
...
| 命令文件 | 触发方式 | 用途 | 是否保留 |
|---------|---------|------|---------|
| `ucc-context-dev.md` | `/ucc-context-dev` | 切换到开发模式 | **保留** |
| `ucc-context-review.md` | `/ucc-context-review` | 切换到审查模式 | **保留** |
| `ucc-context-research.md` | `/ucc-context-research` | 切换到研究模式 | **保留** |
| `ucc-context.md` | `/ucc-context <mode>` | 兼容入口（参数切换） | **保留** |
| `ucc-plan.md` | `/ucc-plan` | 实现规划 | **保留** |
| `ucc-tdd.md` | `/ucc-tdd` | TDD 开发 | 按需保留 |
| `ucc-code-review.md` | `/ucc-code-review` | 代码审查 | **保留** |
| `ucc-build-fix.md` | `/ucc-build-fix` | 构建修复 | **保留** |
| `ucc-update-docs.md` | `/ucc-update-docs` | 文档同步 | 按需保留 |
| `ucc-e2e.md` | `/ucc-e2e` | E2E 测试 | 按需保留 |
| `ucc-verify.md` | `/ucc-verify` | 验证检查 | **保留** |
| `ucc-go-review.md` | `/ucc-go-review` | Go 审查 | 使用 Go 则保留 |
| `ucc-go-test.md` | `/ucc-go-test` | Go TDD 工作流 | 使用 Go 则保留 |
| `ucc-go-build.md` | `/ucc-go-build` | Go 构建修复 | 使用 Go 则保留 |
| `ucc-javascript-review.md` | `/ucc-javascript-review` | JS/TS 审查 | 使用 JS/TS 则保留 |
| `ucc-typescript-fullstack-review.md` | `/ucc-typescript-fullstack-review` | TS 全栈审查 | 前后端同仓则保留 |
| `ucc-sessions.md` | `/ucc-sessions` | 会话管理 | 按需保留 |
| `ucc-test-coverage.md` | `/ucc-test-coverage` | 覆盖率分析 | **保留** |
| `ucc-learn.md` | `/ucc-learn` | 会话模式提取 | 按需保留 |
| `ucc-checkpoint.md` | `/ucc-checkpoint` | 检查点管理 | 按需保留 |
| `ucc-skill-create.md` | `/ucc-skill-create` | 从 git 历史生成技能 | 按需保留 |
| `ucc-refactor-clean.md` | `/ucc-refactor-clean` | 死代码清理 | 按需保留 |
| `ucc-db-review.md` | `/ucc-db-review` | 数据库审查 | 涉及 DB 则保留 |
| `ucc-design-doc.md` | `/ucc-design-doc` | 设计文档生成 | 需要设计文档保留 |
| `ucc-delivery-doc.md` | `/ucc-delivery-doc` | 交付文档生成 | 项目交付时保留 |
| `ucc-quality-gate.md` | `/ucc-quality-gate` | 质量门禁检查 | **保留** |

### 5.2 工作模式切换

推荐使用显式命令快速切换工作模式（更便于团队和个人理解）：

```bash
/ucc-context-dev       # 开发模式：先写代码，后解释；优先可工作方案
/ucc-context-review    # 审查模式：先读代码，再评论；按严重性排序问题
/ucc-context-research  # 研究模式：先理解，后行动；广泛阅读后再下结论
```
...
| 场景 | 推荐模式 |
|------|---------|
| 快速迭代新功能 | `/ucc-context-dev` |
| 代码审查 PR | `/ucc-context-review` |
| 学习新技术 | `/ucc-context-research` |
| 调试复杂问题 | `/ucc-context-research` |
| 重构代码 | `/ucc-context-review` |

### 5.3 命令的两种类型

命令分为 **自包含** 和 **委派代理** 两种：

**自包含命令** — 指令直接写在命令文件中，Claude 在当前会话执行：
...
**委派代理命令** — 通过 `context: fork` + `agent:` 把任务交给子代理执行：

```yaml
---
description: 生成并执行 E2E 测试，输出可复现结果。
context: fork
agent: e2e-runner
---
...
| 字段 | 含义 | 说明 |
|------|------|------|
| `context: fork` | 在子代理中执行 | 必须搭配 `agent:` 使用 |
| `agent: <名称>` | 指定使用哪个代理 | 对应 `agents/<名称>.md` 文件 |

> ⚠️ **重要：** 如果命令需要委派到代理执行，必须同时写 `context: fork` 和 `agent:`，否则会报 `Agent type 'generic' not found` 错误。

**当前命令类型分布：**

| 类型 | 命令 |
|------|------|
| 自包含 | `ucc-code-review`、`ucc-verify`、`ucc-go-test`、`ucc-refactor-clean`、`ucc-test-coverage` 等 |
| 委派代理 | `ucc-plan`→`planner`、`ucc-e2e`→`e2e-runner`、`ucc-build-fix`→`build-error-resolver` 等 11 个 |
...
| `design-doc-patterns/` | 设计文档模板 | 需要设计文档保留 |
| `delivery-patterns/` | 交付文档模板 | 项目交付时保留 |
| `docker-patterns/` | Docker 容器化 | 使用 Docker 则保留 |
| `deployment-patterns/` | 部署策略 | 生产环境部署保留 |
...
cp -r README.md rules agents commands skills contexts hooks scripts mcp-configs docs tests your-project/.claude/
cp hooks/project-settings.json your-project/.claude/settings.json

# 2. 删除不需要的文件
cd your-project/.claude

# Windows 命令（高风险：会删除文件/目录，执行前请确认当前目录与目标路径）
rmdir /s /q rules\golang
del agents\doc-updater.md agents\e2e-runner.md agents\tdd-guide.md
del commands\ucc-update-docs.md commands\ucc-e2e.md commands\ucc-tdd.md
...
rm commands/ucc-update-docs.md commands/ucc-e2e.md commands/ucc-tdd.md
...
del commands\ucc-e2e.md

# macOS/Linux 命令
rm -rf rules/golang
rm agents/e2e-runner.md
rm commands/ucc-e2e.md
...
> 复制以下内容到 Go 微服务项目根目录的 `CLAUDE.md`，根据实际服务自定义。

```markdown
# Go 微服务 — 项目 CLAUDE.md

## 项目概述
技术栈: Go 1.22+, MySQL/SQLite, gRPC + REST, Docker, Wire (DI)
架构: 整洁架构（domain → repository → service → handler）
...
**常用命令：** `/ucc-go-test` → `/ucc-go-review` → `/ucc-go-build`

---

## 附录 B：项目模板 — Vue + Node 应用

> 复制以下内容到 Vue+Node 项目根目录的 `CLAUDE.md`，根据实际应用自定义。

```markdown
# Vue + Node 应用 — 项目 CLAUDE.md

## 项目概述
前端: Vue 3 (Composition API), TypeScript, Vite, Pinia
后端: Node.js, Express/Fastify, TypeScript, MySQL
架构: 前后端分离，RESTful API，JWT 认证

## 关键规则
### 前端
- 使用 `<script setup>` 语法
- 组合式函数放 composables/，命名 use 开头
- Props 必须用 `defineProps<T>()`
...
## 文件结构
### 前端
src/{components,views,composables,stores,services,types}/

### 后端
src/{controllers,services,repositories,middlewares,routes,validators}/
migrations/
```

**常用命令：** `/ucc-tdd` → `/ucc-code-review` → `/ucc-e2e`

---

## 附录 C：配置技术规范 (Frontmatter)

如果你需要扩展或自定义本配置集，请遵循以下 YAML Frontmatter 规范：
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `name` | string | 代理名称（全局唯一） |
| `description` | string | 代理功能简述 |
| `tools` | array | 允许使用的工具列表（如 `["Read", "Write", "Bash", "Edit", "Grep", "Glob"]`） |
| `model` | string | 建议使用的模型（如 `sonnet`, `opus`） |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `description` | string | 斜杠命令说明（显示在自动补全列表中） |
| `context` | string | 设为 `fork` 表示在独立子代理中执行 |
| `agent` | string | 当 `context: fork` 时，指定使用的代理名 |
| `allowed-tools` | array | 允许该命令直接使用的工具列表（非委派模式下使用） |

> 💡 **提示：** 命令中请使用 `allowed-tools`（连字符），不要使用已废弃的 `allowed_tools`。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `name` | string | 技能唯一标识符（建议小写+连字符） |
| `description` | string | 技能描述，Claude 根据此描述判断何时激活该技能 |
